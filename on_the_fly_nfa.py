...
def get_next_state(current_state, fluents):
    new_state = delta(current_state, fluents)
...
def run_on_the_fly_nfa(s0, trace):
    ...
    for fluents in trace:
        if OR_STATE_SEPARATOR in current_state:
            ...
            for state in split:
                n_state = get_next_state(state, fluents)
                if n_state == TRUE:
                    next_state = TRUE
                    break
                if n_state != FALSE:
            ...
            next_state = get_next_state(current_state, fluents)
        if next_state == TRUE:
            return True
        if next_state == FALSE:
            return False
